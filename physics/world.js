...
// classes/physics/world.js
// classes/physics/world.js
/*
import Ball from "./ball"; // افتراض وجود هذا الملف
import Vector from "./vector"; // افتراض وجود هذا الملف

export default class World {
  constructor(gravity, height, temperature, wind_speed, wind_angle) {
    this.gravity = gravity;
    this.height = height;
  ...
  // تم تحديث الدالة لتتطابق مع World.add(physicsBall)
  add(ball) {
    if (ball instanceof Ball) {
      this.balls.push(ball);
    } else {
      console.error("Invalid object added to the world. Expected an instance of Ball.");
    }
  }

  // تم تحديث الدالة لتتطابق مع world.remove(physicsBall)
  remove(ball) {
  ...
  update(time) {
    this.balls.forEach((ball) => {
      // افتراض أن Ball.update موجود
      ball.update(time, this.gravity, this.height, this.temperature, this.wind_speed, this.wind_angle);
    });
  }
