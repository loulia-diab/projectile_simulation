...
// classes/physics/ball.js
/*
import Vector from "./vector"; // ✅ استيراد الفئة الجديدة
import * as THREE from "three";
import { Vector3 } from "three";
...
class Ball {
  constructor(
    ...
    // خصائص الكرة (المادية)
    this.geometry = new THREE.SphereGeometry(raduis, 32, 32);
    this.material = new THREE.MeshStandardMaterial({ color: 0xffffff });
    this.mesh = new THREE.Mesh(this.geometry, this.material);
    
    this.scene = scene;
    
    // إضافة الكرة للمشهد وتفعيل الظلال
    ...
    // الخصائص الفيزيائية
    this.position = position;
    this.velocity = new Vector(0, 0, 0); // ✅ استخدام الفئة الجديدة
    this.velocity.inits(speed, angleXY, angleXZ);
    this.type = type;
    this.drag_coeff = drag_coeff;
    this.rolling = false;
    ...
    // خصائص الدوران
    this.angular_velocity = angular_velocity || new Vector(0, 0, 0); // ✅ استخدام الفئة الجديدة
    
    this.rotateAxes = new Vector( // ✅ استخدام الفئة الجديدة
      this.angular_velocity.getX() > 0 ? 1 : 0,
      this.angular_velocity.getY(),
      this.angular_velocity.getZ()
    );
    ...
    // ✅ تحديث موقع الـ Mesh بناءً على المتجه الخاص
    this.mesh.position.set(this.position.getX(), this.position.getY(), this.position.getZ());
    
    console.log("Ball position after init:", this.mesh.position);
  }
  ...
  update(time, gravity, air_rho, wind_velo) {
    ...
    let liftForce = this.lift_force(air_rho);
    let buoynancyForce = this.buoyancy_force(air_rho, gravity);
    let totalForce = new Vector( // ✅ استخدام الفئة الجديدة
      dragForce.getX() + windForce.getX() + liftForce.getX(),
      gravityForce.getY() +
        dragForce.getY() +
        liftForce.getY() +
    ...
    //Linear Movement
    if (this.rolling) gravityForce.setY(0);
    let acc = new Vector( // ✅ استخدام الفئة الجديدة
      totalForce.getX() / this.mass,
      totalForce.getY() / this.mass,
      totalForce.getZ() / this.mass
    );
    ...
    if (!this.rolling) {
      this.position._x += this.velocity.getX() * time * 10;
      this.position._y += this.velocity.getY() * time * 10;
      this.position._z -= this.velocity.getZ() * time * 10;
    } else {
      this.position._x -=
        Number(this.angular_velocity._z.toFixed(2)) * this.raduis * time * 10;
      this.position._z +=
    ...
    //Total Torques
    let rollingTrouqe = new Vector(0, 0, 0); // ✅ استخدام الفئة الجديدة
    let torque = new Vector3();
    if (this.rolling) {
      rollingTrouqe = this.getTotalRollingTrouqe(
        gravity,
  ...
  gravity_force(gravity) {
    return new Vector(0, -gravity * this.mass, 0); // ✅ استخدام الفئة الجديدة
  }

  drag_force(rho) {
    let velocitySquere = this.velocity.squere();

    let normalize = this.velocity.clone().normalize();
    let drag = new Vector( // ✅ استخدام الفئة الجديدة
      ((velocitySquere * -1) / 2) *
        this.drag_coeff *
        rho *
        this.area *
  ...
  wind_force(rho, wind_velo) {
    ...
    let wind = new Vector( // ✅ استخدام الفئة الجديدة
      ((velocitySquere * 1) / 2) * rho * this.area * normalize.getX(),
      0,
      ((velocitySquere * 1) / 2) * rho * this.area * normalize.getZ()
    );
  ...
  lift_force(rho) {
    ...
    let rotateAxes = this.rotateAxes || new Vector(0, 0, 0); // ✅ استخدام الفئة الجديدة
    let cross = rotateAxes.clone().cross(this.velocity);

    let lift = new Vector( // ✅ استخدام الفئة الجديدة
      ((velocitySquere * 1) / 2) * lift_coeff * rho * this.area * cross.getX(),
      ((-velocitySquere * 1) / 2) * lift_coeff * rho * this.area * cross.getY(),
      ((velocitySquere * 1) / 2) * lift_coeff * rho * this.area * cross.getZ()
    );
  ...
  viscousTorque() {
    let v = new Vector( // ✅ استخدام الفئة الجديدة
      -this.angular_velocity.getX(),
      -this.angular_velocity.getY(),
      -this.angular_velocity.getZ()
    );
  ...
  getTotalRollingTrouqe(gravity, windForce, time) {
    ...
    let result = new Vector( // ✅ استخدام الفئة الجديدة
      Number((-windForce._z * this.raduis * 2).toFixed(2)),
      0,
      Number((windForce._x * this.raduis * 2).toFixed(2))
    );
  ...
  // هذه الدالة تحتاج لمزيد من العمل والربط مع Raycaster
  // سنناقشها لاحقاً
  fraction(object) {
    let tempArray = this.intersectsObjects.filter(
      (element) => element === object.object
